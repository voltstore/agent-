...
async function initSchedule() {
  ...
  try {
    ...
    cronJobs.sora = cron.schedule(
      ...
      async () => { const c = await fbGet('settings'); if (c?.mode === 'manual') return; await runSora(); },
      { timezone: 'Asia/Riyadh' }
    );
    cronJobs.deep = cron.schedule(
      timeToCron(sch.deepStart || '10:00', day),
      async () => { const c = await fbGet('settings'); if (c?.mode === 'manual') return; await runDeep(); },
      { timezone: 'Asia/Riyadh' }
    );
    cronJobs.gold = cron.schedule(
      timeToCron(sch.goldStart || '20:00', day),
      async () => { const c = await fbGet('settings'); if (c?.mode === 'manual') return; await runGold(); },
      { timezone: 'Asia/Riyadh' }
    );
    await logEvent('info', 'الجدول الزمني مفعّل', sch);
  } catch (e) {
    console.error('خطأ في الجدول:', e.message);
...
app.put('/api/leads/:id', requireAuth, async (req, res) => {
  try {
    await fbUpdate(`leads/${req.params.id}`, req.body);
...
app.listen(PORT, async () => {
  console.log(`\nTRIPLE AGENTS — منفذ ${PORT}\n`);
  try {
    const existing = await fbGet('settings');
    if (!existing) { await fbSet('settings', DEFAULT_SETTINGS); }
  } catch { console.warn('تعذّر الاتصال بـ Firebase'); }
  await initSchedule();
